#[doc = "Register `CAPTURE_H` reader"]
pub struct R(crate::R<CAPTURE_H_SPEC>);
impl core::ops::Deref for R {
    type Target = crate::R<CAPTURE_H_SPEC>;
    #[inline(always)]
    fn deref(&self) -> &Self::Target {
        &self.0
    }
}
impl core::convert::From<crate::R<CAPTURE_H_SPEC>> for R {
    fn from(reader: crate::R<CAPTURE_H_SPEC>) -> Self {
        R(reader)
    }
}
#[doc = "Register `CAPTURE_H` writer"]
pub struct W(crate::W<CAPTURE_H_SPEC>);
impl core::ops::Deref for W {
    type Target = crate::W<CAPTURE_H_SPEC>;
    #[inline(always)]
    fn deref(&self) -> &Self::Target {
        &self.0
    }
}
impl core::ops::DerefMut for W {
    #[inline(always)]
    fn deref_mut(&mut self) -> &mut Self::Target {
        &mut self.0
    }
}
impl core::convert::From<crate::W<CAPTURE_H_SPEC>> for W {
    fn from(writer: crate::W<CAPTURE_H_SPEC>) -> Self {
        W(writer)
    }
}
#[doc = "Field `CAPTURE_VALUE` reader - A read reflects the value of the upper 10 bits of the central 42-bits EVTIMER at the time the last capture signal was generated by the CPU (using CMSIS C function \"__SEV();\")."]
pub struct CAPTURE_VALUE_R(crate::FieldReader<u16, u16>);
impl CAPTURE_VALUE_R {
    pub(crate) fn new(bits: u16) -> Self {
        CAPTURE_VALUE_R(crate::FieldReader::new(bits))
    }
}
impl core::ops::Deref for CAPTURE_VALUE_R {
    type Target = crate::FieldReader<u16, u16>;
    #[inline(always)]
    fn deref(&self) -> &Self::Target {
        &self.0
    }
}
impl R {
    #[doc = "Bits 0:9 - A read reflects the value of the upper 10 bits of the central 42-bits EVTIMER at the time the last capture signal was generated by the CPU (using CMSIS C function \"__SEV();\")."]
    #[inline(always)]
    pub fn capture_value(&self) -> CAPTURE_VALUE_R {
        CAPTURE_VALUE_R::new((self.bits & 0x03ff) as u16)
    }
}
impl W {
    #[doc = "Writes raw bits to the register."]
    pub unsafe fn bits(&mut self, bits: u32) -> &mut Self {
        self.0.bits(bits);
        self
    }
}
#[doc = "Capture High Register\n\nThis register you can [`read`](crate::generic::Reg::read), [`write_with_zero`](crate::generic::Reg::write_with_zero), [`reset`](crate::generic::Reg::reset), [`write`](crate::generic::Reg::write), [`modify`](crate::generic::Reg::modify). See [API](https://docs.rs/svd2rust/#read--modify--write-api).\n\nFor information about available fields see [capture_h](index.html) module"]
pub struct CAPTURE_H_SPEC;
impl crate::RegisterSpec for CAPTURE_H_SPEC {
    type Ux = u32;
}
#[doc = "`read()` method returns [capture_h::R](R) reader structure"]
impl crate::Readable for CAPTURE_H_SPEC {
    type Reader = R;
}
#[doc = "`write(|w| ..)` method takes [capture_h::W](W) writer structure"]
impl crate::Writable for CAPTURE_H_SPEC {
    type Writer = W;
}
#[doc = "`reset()` method sets CAPTURE_H to value 0"]
impl crate::Resettable for CAPTURE_H_SPEC {
    #[inline(always)]
    fn reset_value() -> Self::Ux {
        0
    }
}
